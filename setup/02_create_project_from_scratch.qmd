...
5.  Set up a virtual environment for your project
    ...
    3.  To get this virtual environment to appear either run `uv sync` in the Terminal or add a package like "pandas" using `uv add pandas`.
...
To get this new virtual environment to appear you often need to run the following:

```{terminal}
uv sync
```

Alternatively, you can add a package to your virtual environment and it will appear, i.e.:
...
```{=html}
<!--
1. Confirm `pip` is installed 
   1.  Note 1: pip should be installed for users who have previously install Python on their computers; it may not be for new users
   2.  Note 2: while `uv` is designed to replace `pip` some automated processes may automatically use pip to install new packages in VSC and elsewhere. Having pip installed will prevent these processes from raising errors.
   3.  option 1: `uv init --seed`
   4.  option 2: `uv pip install pip`
   5.  option 3: `uv add jupyter` (does that resolve things?) 
-->
```